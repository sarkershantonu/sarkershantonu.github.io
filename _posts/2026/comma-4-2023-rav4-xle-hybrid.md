This blog is to show my Comma Four Setup with my car 

If you want to buy, here is their [website](https://comma.ai/shop)

### Title 
This is a review on Comma-4 device

### Main Project Link
- Product [Link](https://comma.ai/shop/comma-four), currently retails $999, including the Harness cable
- About the product(https://comma.ai/)
- Supported [Cars](https://comma.ai/vehicles) 

### My Purchase Package Includes
- Comma 4 device
- TypeC to TYpeC cable(data + Power)
- Toyota Harness Cable: TypeA Y data cable
- Obd2 power cable : Optional , if you want device to be turned on when car is off. 

### Software Links 
- OEM Software [Openpilot](https://github.com/commaai/openpilot)
- [FrogPilot](https://github.com/FrogAi/FrogPilot)
- [DragonPilot](https://github.com/dragonpilot/dragonpilot)
- [SunnyPilot](https://github.com/sunnypilot/sunnypilot)
- [HGKP]()
- [Bluepilot]()
- [OPGM]

### My Personal Review, please see this post

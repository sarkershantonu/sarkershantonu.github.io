...
When we add a  JDBC Connection Configuration
```-> Thread group ->(right click)add->Config Element)-> JDBC Connection Configuration``` , we will find These
...
any number = The max count of thread share this connection.
To ensure threads don't wait on each other. 
Note: on sharing, thread number = this number
...
- **Max Connection Age (ms)***: Logically, connection lifetime for the validation query.( need more experiment on exact functionality). Default 5000.
...
ALL * is Mandatory fields 

### Why we use this ? 
As it is the configuration for database connection. It will be used for any request that needs a database.Different database's JDBC driver should be located in \lib or should be added to class path(of test plan).
